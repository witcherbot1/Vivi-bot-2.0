...
let handler = async (m, { conn, usedPrefix, args, command, text }) => {
  if (!text) throw `*❐┃يجب عليك ادراج رابط TikTok┃❗❯*`;
...
await conn.sendMessage(m.chat, { react: { text: '⏳', key: m.key } })
  ...
  try {
    ...
    conn.sendFile(m.chat, mediaURL, '', '> *أبرأ ذمتي تمامًا من أي محتوى، مرئي أو مسموع، قد يصادفك في هذه اللحظة.*', m, false, { mimetype: 'video/mp4' });
  } catch (error) {
